...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        ArrayDeque<Integer> queueFriends = new ArrayDeque<Integer>();
        queueFriends.add(index);

        for (; deep >= 0; deep--) {
            int size = queueFriends.size();
            for (int n = 0; n < size; n++) {
                Integer friend = queueFriends.pollFirst();
                if (friend != index) friends.add(friend);

                for (int j = 0; j < friend; j++) {
                    if (humanRelationships[friend][j]) queueFriends.addLast(j);
                }
                for (int i = friend + 1; i < humanRelationships.length; i++) {
                    if (humanRelationships[i][friend]) queueFriends.addLast(i);
                }
            }
        }
        return friends;
